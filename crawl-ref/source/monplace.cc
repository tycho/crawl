...
void spawn_random_monsters()
{
        ...
        && x_chance_in_y((you.num_turns % CYCLE_LENGTH), CYCLE_LENGTH))
    {
        ...
        mgen_data mg(WANDERING_MONSTER, BEH_SEEK, 0, 0, coord_def(), MHITYOU);
        mg.proximity = prox;
        mons_place(mg);
        viewwindow(true, false);
        return;
...
int place_monster(mgen_data mg, bool force_pos)
{
    ...
    if (!mg.use_position())
    {
        ...
        while (true)
        {
            ...
            if (  mg.behaviour == BEH_SEEK && ((abs(mg.pos.x - (find_floor_item(OBJ_ORBS,ORB_ZOT)->pos.x)) > 5) || (abs(mg.pos.y - (find_floor_item(OBJ_ORBS,ORB_ZOT)->pos.y)) < 13)))
                proxOK = false;
